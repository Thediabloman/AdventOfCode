...
object Day2 {
  ...
  def test(): Unit = {
    val program =
      "1,9,10,3,2,3,11,0,99,30,40,50".split(",").map(_.toInt).toArray
  ...
  def readInput(): Array[Int] = {
    io.Source
      .fromFile("advent/resources/day2/puzzle_1.txt")
      .getLines
      .flatMap(_.split(","))
      .map(_.toInt)
      .toArray
  ...
  def puzzle2(): Unit = {
    ...
    val inputs = Stream
      .from(0)
      .take(100)
      .flatMap(s => Stream.from(0).take(100).map((s, _)))
    val res = inputs
      .map { in =>
