...
object Day7 {
  ...
  def puzzle1(): Unit = {
    val program = io.Source.fromFile("advent/resources/day7/puzzle_1.txt").getLines
      .filter(!_.isEmpty).toList.head.split(",").map(_.toInt)

    val test = "3,31,3,32,1002,32,10,32,1001,31,-2,31,1007,31,0,33,1002,33,7,33,1,33,31,31,1,32,31,31,4,31,99,0,0,0".split(",")
      .map(_.toInt)
