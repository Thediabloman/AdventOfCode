...
object Day4 {
  ...
  def puzzle1(): Unit = {
    val res = Stream.from(372037).take(905157-372037).map(_.toString)
      .filter(increases)
      .filter(hasPair)
      .size
    println("res = " + res)
  ...
  def noTriple(input: List[Char]): Boolean = {
    input match {
      ...
      case _ =>
        input.sliding(4).exists{ case h1 :: h2 :: h3 :: h4 :: Nil  =>
          h2 == h3 && h1 != h2 && h3 != h4 
        }
    }
  }
  ...
  def increases(input: String): Boolean = {
    ...
    val res = input.toCharArray.foldLeft(('0', true)){ case ((prev, cond), current) =>
