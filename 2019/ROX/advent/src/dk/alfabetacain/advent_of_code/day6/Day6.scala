...
object Day6 {
  ...
  def puzzle1(): Unit = {
    val lines = io.Source.fromFile("advent/resources/day6/puzzle_1.txt").getLines
  ...
  def puzzle2(): Unit = {
    ...
    println("puzzle 2 distance: " + (withDistances.get(graph.get("SAN").get).get-2))
  }
  ...
  def buildGraph(lines: List[String]): Map[String, Vertex] = {
    ...
    lines.foldLeft[Map[String, Vertex]](Map.empty){ (map, line) =>
      ...
      val first: Vertex = map.get(parts(0)).getOrElse(Vertex(parts(0), Set.empty))
      val second: Vertex = map.get(parts(1)).getOrElse(Vertex(parts(1), Set.empty))
      (map + (parts(0) -> first.copy(neighbours = first.neighbours + second.name))) + (parts(1) -> second.copy(neighbours =second.neighbours + first.name))
    }
  }
  ...
  def bfs(from: Vertex, all: Map[String, Vertex]): Map[Vertex, Int] = {
    ...
    while (!toBeExplored.isEmpty) {
      val res = toBeExplored.foldLeft[(Map[Vertex, Int], Set[Vertex])]((explored, Set.empty)){ case ((m, next), vertex) =>
        m.get(vertex) match {
          case None => 
            (m + (vertex -> i), next ++ vertex.neighbours.map(n => all.get(n).get))
          case Some(_) => 
            (m, next)
        }
      }
